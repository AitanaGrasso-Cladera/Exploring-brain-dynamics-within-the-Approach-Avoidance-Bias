%% Unfold for picture onset
eeglab;
init_unfold
%% Get information about files
participants = [];
tmp = dir(fullfile(dataPath,filesep,'Step_5'));
inx = 1;
...
CN = nan(size(participants,2)-length(toExclude),64,900);
CP = nan(size(participants,2)-length(toExclude),64,900);
IN = nan(size(participants,2)-length(toExclude),64,900);
IP = nan(size(participants,2)-length(toExclude),64,900);
Mov = nan(size(participants,2)-length(toExclude),64,900);
...
for pIx = 1:size(participants,2)
    if ismember(pIx,toExclude)
        ...
        % Load EEG data
        thisEEGFile = [dataPath,filesep,'Step_5',filesep,'noBurst',filesep,participants(pIx).name];
        ...
        for j = 1:length(eventsIdx)
            ...
            if sum(ismember(excluding,target)) == 1
                ...
                EEG.event(target).valenceType = (EEGtrials.valenceType(1,trial));
                EEG.event(target).conditionType = (EEGtrials.conditionType(1,trial));
                EEG.event(target).currentType = (EEGtrials.currentType(1,trial));
            end
        ...
        cfgDesign = [];
        cfgDesign.eventtypes = {'Picture','Joystick'};
...
% % Exclude this participant because it has less than 50% of correct data
 ...
 Mov(23,:,:) = [];
% 
save(['/Volumes/Aitanas_SSD/AAB_EEG/unfold/','unfoldResults_January_PicOnset.mat'],"time","IP","IN","CP","CN")
% 
rmpath(eegLabFolder)
rmpath(dataPath)
rmpath(unfoldFolder)
